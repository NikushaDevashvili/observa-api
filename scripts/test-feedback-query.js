...
const TINYBIRD_HOST =
  process.env.TINYBIRD_HOST || "https://api.europe-west2.gcp.tinybird.co";
...
async function testFeedbackQuery() {
  try {
    ...
    results.forEach((row, index) => {
      ...
      console.log(
        "attributes_json raw:",
        String(row.attributes_json).substring(0, 300)
      );
      ...
      try {
        ...
        if (attrs && attrs.feedback) {
          console.log(
            ...
            JSON.stringify(attrs.feedback, null, 2)
          );
        } else {
          ...
          console.log(
            "Full attributes:",
            JSON.stringify(attrs, null, 2).substring(0, 500)
          );
        }
      } catch (e) {
