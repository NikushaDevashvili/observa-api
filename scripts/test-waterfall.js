...
async function sendWaterfallTestTrace() {
  ...
  const traceData = {
    ...
    context: "[CONTEXT] Refund Policy: Refunds are allowed within 30 days only. No refunds for digital items. All refunds must be requested through the customer portal.",
    response: "Refunds are allowed within 30 days only. No refunds for digital items. All refunds must be requested through the customer portal.",
    model: "gpt-4o-mini",
    tokensPrompt: 150,
    tokensCompletion: 45,
    tokensTotal: 195,
  ...
  try {
    ...
    console.log(`   Tokens: ${traceData.tokensTotal} (${traceData.tokensPrompt} input, ${traceData.tokensCompletion} output)`);
    ...
    if (response.ok) {
      ...
      console.log(`💡 Or in production: https://your-app.vercel.app/dashboard/traces/${traceId}`);
