...
export class AnalysisService {
  ...
  static async analyzeTrace(trace: TraceEvent): Promise<void> {
    try {
      ...
      if (!process.env.ANALYSIS_SERVICE_URL) {
        console.log(
          `[Analysis] ANALYSIS_SERVICE_URL not set, skipping analysis for trace ${trace.traceId}`
        );
        return;
      }
      ...
      console.log(
        `[Analysis] Analysis completed for trace ${trace.traceId}, storing results...`
      );
      ...
      console.log(
        `[Analysis] Successfully stored analysis results for trace ${trace.traceId}`
      );
    } catch (error) {
      console.error(
  ...
  private static async storeAnalysisResults(
    ...
    // Check if conversation columns exist
    ...
    try {
      ...
      console.warn(
        "[AnalysisService] Could not check for conversation columns, assuming they exist:",
        err
      );
      hasConversationColumns = true; // Assume they exist
    }
    ...
    if (hasConversationColumns) {
      ...
      await query(
        ...
        ON CONFLICT (trace_id) DO UPDATE SET
          ...
          -- Preserve trace data fields (don't overwrite existing trace data)
          ...
          -- Preserve conversation tracking fields (don't overwrite them)
        ...
        [
          ...
          // Conversation tracking fields (preserve from existing row if not in trace)
      ...
      // Fallback query without conversation columns
      await query(
        `INSERT INTO analysis_results (
          trace_id, tenant_id, project_id, analyzed_at,
          is_hallucination, hallucination_confidence, hallucination_reasoning,
